...
print("Pré-processamento concluído. Início do treinamento...")

for row in matrix_config:
    print(f"\nTreinando MLP com camadas ocultas: {row}")

    keras_clf = KerasClassifier(model=build_model, model__hidden_layers=row, epochs=50, batch_size=32, verbose=0)
    ...
    plt.figure()
    plt.plot(train_sizes, train_scores_mean, 'o-', label='Acurácia Treino')
    plt.plot(train_sizes, test_scores_mean, 'o-', label='Acurácia Validação')
    plt.title(f"Curva de Aprendizado - Arquitetura {row}")
    ...
    print("Curva de aprendizado gerada. Iniciando o ajuste do modelo...")
    ...
    plt.xlabel("Predição")
    plt.ylabel("Real")
    plt.title(f"Matriz de Confusão - Arquitetura {row}")
    plt.tight_layout()
    plt.show()
